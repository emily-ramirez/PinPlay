...
### 5. Deployment
To deploy our application to production, we serve it as a static application on a linux-based web host. Accordingly, we reserved a web server and domain name, used a File Transfer Protocol application to move our project's production directory to our host, and implemented advised production settings for a Django server.  
